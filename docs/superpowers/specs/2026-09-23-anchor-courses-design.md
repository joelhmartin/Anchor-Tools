...
| Course access type `course_access_type` (§6.3) | Values: `open` (any logged-in user may enroll), `closed` (enrolment only via API/admin/integration), `roles` (auto-enrol holders of listed roles; see §3). | Makes the events contract concrete. |
...
### 3.1 Roles are the currency

- **Consuming:** `course_access_type = roles` lists role slugs (`course_auto_enroll_roles`). `Integrations\Events` listens to `anchor_events_access_granted( $event_id, $user_id, $source )` **and** to core `add_user_role`/`set_user_role`, and calls `anchor_courses_enroll_user( $user_id, $course_id, ['source'=>'role', 'source_id'=>$role] )` when a listed role is gained. Enrolment is idempotent (unique `(user_id, course_id)`). **The picker offers only `anchor_event_*` and `anchor_course_*` roles plus roles an admin created by hand; WordPress's built-in roles (`administrator`, `editor`, `author`, `contributor`, `subscriber`) and WooCommerce's (`customer`, `shop_manager`) are excluded** unless `anchor_courses_auto_enroll_role_choices` adds them back. The listener additionally ignores any role not on the saved list, so a stray `set_user_role` from account creation can never enrol anyone. Reason: `set_user_role` fires on every new account, so listing `customer` would enrol every shopper on the site; listing `subscriber` would enrol every registrant. Losing the role does **not** un-enrol by default (`anchor_courses_role_loss_policy` filter: `keep|expire|cancel`, default `keep`), matching the events module's "access outlives the event" stance.
- **Producing:** on course completion, `Support\Roles::grant_completed( $user_id, $course_id )` mints (lazily) and adds role **`anchor_course_{course_id}`**, display name `Completed: {course title}`, no capabilities. Renamed on title change; never auto-deleted; "Delete role" action in the course editor. This role is what an event lists in `required_roles` as a prerequisite, and what the file manager can gate on.
- Note: an event only mints `anchor_event_{id}` when its `access_role_enabled` switch is on (stream spec §3.1); an in-person event whose attendees should auto-enrol here needs that switch ticked in the event's Access section. The course editor's help text says so.
- **Prerequisites between courses** (`course_prerequisites`, brief §6.3) are also role slugs, resolved by the same `Roles` helper, so a course may require another course's completion role **or** an event's attendance role. One mechanism.

### 3.2 Actions and filters we hook

| Events surface | Courses use |
|---|---|
| `anchor_events_access_granted/revoked` | auto-enrol (§3.1); revoked → policy filter |
...
- **Phase 5 (integration layer)** includes the events contract (§3 here) and the live-session lesson; **Phase 6** is WooCommerce.
...
Everything in brief §33, plus: PDF certificates (Phase 4b), webhooks, `external_event` completion, instructor roles, and any stream rendering inside courses.
