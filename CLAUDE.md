...
| `testimonials` | `Anchor_Testimonials_Module` | CPT (non-public, `anchor_testimonial`) |
| `speakers` | `Anchor_Speakers_Module` | CPT (public, `anchor_speaker`) |
...
- **Shared front-end primitives**: A reusable client-side widget (lightbox, carousel) belongs in `assets/shared/` and is registered under a plain handle (`anchor-lightbox`, `anchor-carousel`) by `includes/class-anchor-shared-assets.php`; modules that need it enqueue by handle instead of shipping their own copy. Shared video URL parsing (YouTube/Vimeo id, start time, thumbnail lookup) lives in `includes/class-anchor-video-url.php` (`Anchor_Video_URL::parse()` / `::thumbnail()`). This is where new cross-module front-end primitives should go.
