...
class Anchor_Schema_Admin {
    ...
    public function register_settings(){
        ...
        add_settings_section('anchor_schema_modules', __('Modules', 'anchor-schema'), function(){
            echo '<p>' . esc_html__('Enable or disable bundled Anchor Tools modules. Disabling hides menus, CPTs, and shortcodes for that module.', 'anchor-schema') . '</p>';
        }, 'anchor_schema_settings');
        ...
        add_settings_field('reviews_last_fetch', __('Last Successful Fetch', 'anchor-schema'), function(){
            $opts = $this->get_settings();
            $place_id = $opts['reviews_google_place_id'] ?? '';
            $last = '';
            if ( class_exists( 'Anchor_Reviews_Manager' ) && $place_id ) {
    ...
    private function get_settings(){
        ...
        if ( ! isset( $opts['google_api_key'] ) ) {
            $opts['google_api_key'] = '';
        }
        if ( ! isset( $opts['reviews_google_place_id'] ) ) {
            $opts['reviews_google_place_id'] = '';
        }
    ...
    public function render_modules_field(){
        if ( ! function_exists('anchor_tools_get_available_modules') ) {
            echo '<p>' . esc_html__('No modules detected.', 'anchor-schema') . '</p>';
            return;
        }
        $opts = $this->get_settings();
        $modules = anchor_tools_get_available_modules();
        echo '<div class="anchor-schema-modules">';
        foreach ( $modules as $key => $module ) {
            $enabled = ! empty( $opts['modules'][ $key ] );
            printf(
                '<label style="display:flex;gap:8px;align-items:flex-start;margin-bottom:12px;"><input type="checkbox" name="%1$s[modules][%2$s]" value="1" %3$s/> <span><strong>%4$s</strong><br><small>%5$s</small></span></label>',
                esc_attr( self::OPTION_KEY ),
                esc_attr( $key ),
    ...
    public function render_settings_page(){
        ...
        if ( isset( $_GET['anchor_reviews_refresh'] ) ) {
            ...
            if ( $status === 'success' ) {
                add_settings_error( 'anchor_schema_settings', 'reviews_refresh', __( 'Reviews refreshed successfully.', 'anchor-schema' ), 'updated' );
            } elseif ( $status === 'fail' ) {
                add_settings_error( 'anchor_schema_settings', 'reviews_refresh', __( 'Reviews refresh failed. Check API key and Place ID.', 'anchor-schema' ), 'error' );
            }
        }
        settings_errors('anchor_schema_settings');
